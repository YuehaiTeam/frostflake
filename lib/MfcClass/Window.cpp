...
Window::Window(Control *parent, std::wstring name, int width, int height)
...
void Window::setOnCustomMessage(f_onCustomMessage call) {
    mOnCustomMessage = call;
}
...
Window::Window(Control *parent, std::wstring name, DWORD flags, int width, int height)
...
LRESULT Window::execute(UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
    case WM_DPICHANGED:
        handleDpiChange(HIWORD(wParam),(RECT*)lParam);
        break;
    case WM_CONTEXTMENU:
        if (ctlExists(wParam)) {
    ...
    default:
        if (mOnCustomMessage) {
            mOnCustomMessage(this, uMsg, wParam, lParam);
        }
    }
