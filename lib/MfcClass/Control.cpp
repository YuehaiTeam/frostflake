...
void Control::setSize(int width, int height) {
    ...
    if (mCreated) {
        SetWindowPos(mHwnd, NULL, 0, 0, width * mDpi / 96, height * mDpi / 96, SWP_NOMOVE);
        updateClientRect();
    }
}
...
void Control::setRect(RECT rect) {
    ...
    if (mCreated) {
        SetWindowPos(mHwnd, NULL, mX, mY, mWidth * mDpi / 96, mHeight * mDpi / 96, NULL);
        updateClientRect();
    }
}

void Control::setRectRaw(RECT rect) {
    mX = rect.left;
    mY = rect.top;

    if (mCreated) {
        SetWindowPos(mHwnd, NULL, mX, mY, rect.right, rect.bottom, NULL);
        updateClientRect();
    }
}
...
void Control::setFont(std::wstring str) {
...
void Control::setText(std::wstring txt) {
    mText = txt;
    if (mCreated)
...
Control::Control(Control *parent, std::wstring name, int width, int height) {
    globalClassInit();
...
Control::Control(Control *parent, DWORD flags, std::wstring name, int width, int height) {
...
void Control::handleDpiChange(unsigned long newDpi, RECT *suggestedRect) {
    if (mDpi == newDpi)
        return;
    mDpi = newDpi;
    updateFont();
...
void Control::updateWindowRect() {
    ...
    if (GetWindowRect(mHwnd, &rect)) {
        mX = rect.left;
        mY = rect.top;
        mWidth = (rect.right - rect.left) / (mDpi / 96);
        mHeight = (rect.bottom - rect.top) / (mDpi / 96);
    }
}
...
void Control::updateFont() {
    if (mFont != nullptr)
        DeleteObject(mFont);
    LOGFONT mlf2 = mLogFont;
    mlf2.lfHeight = mlf2.lfHeight * (mDpi / 96);
    mFont = CreateFontIndirect(&mlf2);
...
std::wstring Control::pullWindowText() {
