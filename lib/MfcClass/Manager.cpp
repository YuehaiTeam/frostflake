...
int Dialog(std::wstring text, std::wstring title, DWORD flags)
{
	return MessageBox(NULL, text.c_str(), title.c_str(), flags);
}

std::wstring OpenFileDialog(UINT size, const COMDLG_FILTERSPEC* c_rgFileTypes)
{
	std::wstring path;
...
std::vector<std::wstring> OpenMultiFileDialog(UINT size, const COMDLG_FILTERSPEC* c_rgFileTypes)
{
...
std::wstring SaveFileDialog(UINT size, const COMDLG_FILTERSPEC* c_rgFileTypes, std::wstring fName)
{
	std::wstring path;
	if (FAILED(CoInitializeEx(NULL, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE)))
		return path;
	...
	hr = pFileSave->SetFileName(fName.c_str());
